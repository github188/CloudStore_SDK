...
#define PARA_CLOUD_SW_SET	"setCldSw"
#define PARA_CLOUD_SW_GET	"getCldSw"
#define PARA_SID			"sid"


CLOUD_Controler::CLOUD_Controler(const char* remoteHost, const int httpPort, const char* baseResource)
...
CLOUD_Controler::~CLOUD_Controler()
{
	try
	{
		if (__HttpTrans)
...
int CLOUD_Controler::setCloudSwitch(const char* userSid, const int cloudSw)
{
	...
	/* From HTTP Header */
	(void)snprintf(resource, sizeof(resource), "%s?" PARA_SID "=%s&" PARA_CLOUD_SW_SET "=%d", 
					__BaseResource, userSid, cloudSw);
...
int CLOUD_Controler::getCloudSwitch(const char* userSid, int& cloudSw)
{
	CHECK_RET(userSid != NULL, RET_ERROR);
	...
	/* From HTTP Header */
	(void)snprintf(resource, sizeof(resource), "%s?" PARA_SID "=%s&" PARA_CLOUD_SW_GET "=%d", 
					__BaseResource, userSid, cloudSw);
