...
static char* xml_feed( XMLBUF *xml, int (*test)(char) )
{
	...
	while( test(xml->buf[offset]) )
	{
		...
		if(offset >= xml->len)
		{
			...
			tmp = (char *)realloc(ret, size + delta + 1);
...
static void xml_read_attr(struct XMLBUF *xml, XmlNode *node)
{
	...
	while(xml->len)
	{
		...
		n = ++node->nattrib;
		tmp = (char **)realloc(node->attrib, n * 2 * sizeof(char*) );
	...
	while( (xml->read_index < xml->len) || !xml->eof )
	{
		switch(is_special(xml_peek(xml)))
		{
			case XML_OPEN:
				...
				*this_node = xml_new( xml_feed(xml, test_mask));
				if(xml->error)goto xml_parse_malloc;
				xml_skip(xml, XML_SPACE);	// skip any whitespace

				xml_read_attr(xml, *this_node);	// read attributes
			...
			default:	// text node
				*this_node = xml_new(0);
				...
				(*this_node)->attrib = tmp;
				(*this_node)->attrib[1] = NULL;
				stmp = (*this_node)->attrib[0] = xml_feed(xml, test_mask);
